...
imap_dfr(data_chronic_disease_comparison,
  ...
  arrange(order) %>%
  select(-order) %>%
  write.csv(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/Abs_risk_per_chronicity.csv",
            row.names = FALSE)

#Fitting models
#Comparison with counterparts who have chronic diseases

mfull_chronic_disease_comp <- imap_dfr(data_chronic_disease_comparison,
                                       function(data_rheuma_dg, rheuma_cohort_names) {
...
#Comparison with counterparts without chronic diseases

mfull_non_chronic_disease_comp <- imap_dfr(data_non_chronic_disease_comparison,
                                           function(data_rheuma_dg, rheuma_cohort_names) {
...
mfull_chronic_disease_comp %>% 
  mutate(comparison = "Counterparts with chronic diseases") %>%
  bind_rows(mfull_non_chronic_disease_comp %>%
              mutate(comparison = "Counterparts without chronic diseases")) %>%
  ...
  transmute(cohort,
            ...
            estimate = paste(formatC(round(estimate, 2), format = "f", digits = 2),
                             paste0("(", 
                                    ...
                                    ")"))) %>%
  pivot_wider(names_from = c("cohort", "comparison"),
              values_from = "estimate") %>%
  select(-order) %>%
  write.csv(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/HR_per_chronicity.csv",
            row.names = FALSE)

#Graph

mfull_chronic_disease_comp %>%
  mutate(comparison = "Counterparts with \nchronic diseases") %>%
  bind_rows(mfull_non_chronic_disease_comp %>%
              mutate(comparison = "Counterparts without \nchronic diseases")) %>%
  mutate(cohort = case_when(cohort == "Rheumatoid arthritis or ankylosing spondylitis" ~ "Rheumatoid arthritis or axial spondyloarthritis",
                            cohort == "Rheumatoid arthritis" ~  "Rheumatoid arthritis",
                            cohort == "Ankylosing spondylitis" ~ "Axial spondyloarthritis")) %>%
  mutate(cohort = factor(cohort,
                         levels = c("Rheumatoid arthritis or axial spondyloarthritis",
                                    "Rheumatoid arthritis",
  ...
  geom_hline(yintercept = 1, 
             linetype = 2) +
  scale_y_log10(breaks = c(0.25, 0.5, 0.7, 1, 1.5, 2, 3, 5, 9),
                limits = c(0.25, 9)) +
  scale_colour_manual(values = c("#D55E00", "#0072B2"),
                      breaks = c("Counterparts without \nchronic diseases",
                                 "Counterparts with \nchronic diseases")) +
  coord_flip() +
  facet_wrap(~ cohort,
             ncol = 1,
             nrow = 3,
             scales = "free_x") +
  labs(title = "<span style = 'font-size:16pt;'>**Comparison with counterparts**
       <span style = 'color:#D55E00;'>**without other chronic physical health conditions**</span> **or**
       <span style = 'color:#0072B2;'>**with other chronic physical health conditions**</span>",
       x = NULL,
       y = "aHR (95% CI)") +
  theme_classic() +
...
ggsave(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/HR_per_chronicity.eps",
       device = cairo_ps,
       width = 40,
       height = 55,
       units = "cm",
           ...
           imap_dfr(data_rheuma_dg,
                    function(data_rheuma_and_psychiatric_dg, outcome_names) {
                      ...
                      p <- ggsurvplot(surv_fit(Surv(years_until_outcome_or_censoring, outcome) ~ exposure, 
                                      ...
                                      title = paste0(outcome_names, " following ", tolower(rheuma_cohort_names), ", chronic disease comparison"),
                      ...
                      ggsave(plot = plots,
                             filename = paste0("Cumulative_event_plot_", tolower(outcome_names), "_", tolower(rheuma_cohort_names), "_chronic_disease_comparison", ".eps"),
...
#Non-chronic disease comparison

imap_dfr(data_non_chronic_disease_comparison,
         function(data_rheuma_dg, rheuma_cohort_names) {
           ...
           imap_dfr(data_rheuma_dg,
                    function(data_rheuma_and_psychiatric_dg, outcome_names) {
                      ...
                      p <- ggsurvplot(surv_fit(Surv(years_until_outcome_or_censoring, outcome) ~ exposure, 
                                      ...
                                      break.x.by = 5,
                                      legend.labs = c(paste0("People without ", tolower(rheuma_cohort_names)), paste0("People with ", tolower(rheuma_cohort_names))),
                                      title = paste0(outcome_names, " following ", tolower(rheuma_cohort_names), ", non-chronic disease comparison"),
                                      xlab = "Time (years) since the health condition",
                                      ylab = "Cumulative event (95% CI)",
                                      palette = c("blue", "red"),
                                      legend.title = "",
                      ...
                      ggsave(plot = plots,
                             filename = paste0("Cumulative_event_plot_", tolower(outcome_names), "_", tolower(rheuma_cohort_names), "_non_chronic_disease_comparison", ".eps"),
                             path = "path/AZV_rheuma_risk_of_psych_hosp/Results/Cumulative_event_plots",
                             device = cairo_ps,
                             width = 15, 
                             height = 13, 
