...
hospitalizations_1999_2012_rheuma <- map(rheuma_codes_vec,
                                         function(rheuma_codes) {
                                           ...
                                           hospitalizations_1994_2017 %>%
                                             ...
                                             filter(!grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", ZDG) &
                                                      !grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG2) &
                                                      !grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG3) &
                                                      !grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG4) &
                                                      !grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG5)) %>%
                                             filter(VEK >= 18)
                                         })
...
#Excluding individuals with history of hospitalization for the chronic health conditions five or more years before

first_hospitalizations_rheuma_no_chronic_disorder_1999_2012 <- map(first_hospitalizations_rheuma_1999_2012,
                                                                   function(data_rheuma_dg) {
                                                                     
                                                                     data_rheuma_dg %>%
                                                                       anti_join(data_rheuma_dg %>%
                                                                                   transmute(RODCIS2,
                                                                                   ...
                                                                                   inner_join(hospitalizations_1994_2017 %>%
                                                                                              ...
                                                                                              by = c("RODCIS2" = "RODCIS2")) %>%
                                                                                   mutate(past_chronic_disorder_hospitalizations = int_overlaps(interval(washout_start, washout_end),
                                                                                                                                                interval(historic_hospitalization_start, historic_hospitalization_end))) %>%
                                                                                   group_by(RODCIS2) %>%
                                                                                   filter(any(past_chronic_disorder_hospitalizations == TRUE)) %>%
                                                                                   ungroup(),
                                                                                 by = c("RODCIS2" = "RODCIS2"))
                                                                   })
...
#Excluding individuals with history of hospitalization for specific rheumatic disorder and chronic disorders five or more years before
#Exact matching on age, sex, month and year at discharge
#Excluding individuals with residence outside of Czechia
#Randomly select five unexposed counterparts

set.seed(123)
all_matched_sampled_pairs <- map2(first_hospitalizations_rheuma_no_chronic_disorder_1999_2012,
                                  rheuma_codes_vec,
                                  function(data_rheuma_dg, rheuma_codes) {
                                    ...
                                    hospitalizations_unexp_without_past_rheuma_or_chronic_1999_2012 <- hospitalizations_unexp_1999_2012 %>%
                                      anti_join(hospitalizations_unexp_1999_2012 %>%
                                                  ...
                                                  inner_join(hospitalizations_1994_2017 %>%
                                                               filter(grepl(rheuma_codes, ZDG) | 
                                                                        ...
                                                                        grepl(rheuma_codes, DDG4) | 
                                                                        grepl(rheuma_codes, DDG5) |
                                                                        grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", ZDG) |
                                                                        grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG2) |
                                                                        grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG3) |
                                                                        grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG4) |
                                                                        grepl("^I25|^I50|^E11|^J4[1-4]|^J45|^N03|^N11|^N1[8-9]|^N80|^K760|^G4[0-1]|^G2[0-2]", DDG5)) %>%
                                                               transmute(RODCIS2,
                                                                         ...
                                                                         historic_hospitalization_end = ymd(DATUKO)),
                                                             by = c("RODCIS2" = "RODCIS2")) %>%
                                                  mutate(past_rheuma_or_chronic_hospitalizations = int_overlaps(interval(washout_start, washout_end),
                                                                                                                interval(historic_hospitalization_start, historic_hospitalization_end))) %>%
                                                  group_by(RODCIS2, rowid) %>%
                                                  filter(any(past_rheuma_or_chronic_hospitalizations == TRUE)) %>%
                                                  ungroup(),
                                                by = c("RODCIS2" = "RODCIS2", 
                                                       "rowid" = "rowid" ))
                                    ...
                                    matched_pairs <- data_rheuma_dg %>%
                                      ...
                                      inner_join(hospitalizations_unexp_without_past_rheuma_or_chronic_1999_2012 %>%
                                    ...
                                    all_matched_sampled_pairs <- matched_pairs %>%
                                      ...
                                      filter(row_number() %in% sample(1:n(), pmin(n(), 5))) %>%
                                      ungroup()
...
map2(.x = map(first_hospitalizations_rheuma_no_chronic_disorder_1999_2012,
              function(data_rheuma_dg) {
...
map2_dfr(.x = imap(first_hospitalizations_rheuma_no_chronic_disorder_1999_2012,
                   function(data_rheuma_dg, names_rheuma_dg) {
                     
                     data_rheuma_dg %>%
                       summarise(cohort = names_rheuma_dg,
                                 total_n = n())
                   }),
         .y = map(all_matched_sampled_pairs,
                  function(data_rheuma_dg) {
                    ...
                    data_rheuma_dg %>%
                      summarise(matched_n = n_distinct(RODCIS2_exposed))
                  }),
         ~ bind_cols(.x,
                     .y)) %>%
  mutate(unmatched_n = total_n - matched_n) %>%
  write.csv(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/Unmatched_n_non_chronic_disease_comparison.csv",
            row.names = FALSE)

#Combine data of people with rheumatic diseases and their matched counterparts
#Excluded unmatched people with rheumatic diseases

cohorts_baseline <- map2(first_hospitalizations_rheuma_no_chronic_disorder_1999_2012,
                         all_matched_sampled_pairs,
                         function(data_rheuma_dg, data_rheuma_dg_matched_counterparts) {
                           
                           data_rheuma_dg %>%
                             filter(RODCIS2 %in% data_rheuma_dg_matched_counterparts$RODCIS2_exposed) %>%
...
data_non_chronic_disease_comparison <- map(cohorts_baseline_with_outcome,
                                           function(data_rheuma_dg) {
...
save(data_non_chronic_disease_comparison, 
     file = "path/AZV_rheuma_risk_of_psych_hosp/Data/data_non_chronic_disease_comparison.RData")