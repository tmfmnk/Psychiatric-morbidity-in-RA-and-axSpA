                                           ...
                                           map(psychiatric_codes_vec,
                                               function(psychiatric_codes) {
                                                 ...
                                                 hospitalizations_1994_2017 %>%
                                                   ...
                                                   filter(!grepl(psychiatric_codes, ZDG) & 
                                                            !grepl(psychiatric_codes, DDG2) & 
                                                            !grepl(psychiatric_codes, DDG3) & 
                                                            !grepl(psychiatric_codes, DDG4) & 
                                                            !grepl(psychiatric_codes, DDG5)) %>%
                                                   filter(VEK >= 18)
                                               })
                                         })
...
#Excluding individuals with history of hospitalization for specific psychiatric disorder five or more years before

first_hospitalizations_rheuma_no_mental_disorder_1999_2012 <- map(first_hospitalizations_rheuma_1999_2012,
                                                                  function(data_rheuma_dg) {
                                                                    
                                                                    map2(data_rheuma_dg,
                                                                         psychiatric_codes_vec,
                                                                         function(data_rheuma_and_psychiatric_dg, psychiatric_codes) {
                                                                           
                                                                           data_rheuma_and_psychiatric_dg %>%
                                                                             anti_join(data_rheuma_and_psychiatric_dg %>%
                                                                                         transmute(RODCIS2,
                                                                                         ...
                                                                                         inner_join(hospitalizations_1994_2017 %>%
                                                                                                    ...
                                                                                                    by = c("RODCIS2" = "RODCIS2")) %>%
                                                                                         mutate(past_psychiatric_hospitalizations = int_overlaps(interval(washout_start, washout_end),
                                                                                                                                                 interval(historic_hospitalization_start, historic_hospitalization_end))) %>%
                                                                                         group_by(RODCIS2) %>%
                                                                                         filter(any(past_psychiatric_hospitalizations == TRUE)) %>%
                                                                                         ungroup(),
                                                                                       by = c("RODCIS2" = "RODCIS2"))
                                                                         })
                                                                  })
...
#Excluding individuals with history of hospitalization for specific rheumatic and psychiatric disorder five or more years before
#Exact matching on age, sex, month and year at discharge
#Excluding individuals with residence outside of Czechia
#Randomly select five unexposed counterparts

set.seed(123)
all_matched_sampled_pairs <- map2(first_hospitalizations_rheuma_no_mental_disorder_1999_2012,
                                  rheuma_codes_vec,
                                         ...
                                         function(data_rheuma_and_psychiatric_dg, psychiatric_codes) {
                                           ...
                                           hospitalizations_unexp_1999_2012 <- hospitalizations_1994_2017  %>%
                                             ...
                                             filter(!grepl(psychiatric_codes, ZDG) & 
                                                      ...
                                                      !grepl(psychiatric_codes, DDG5)) %>%
                                             filter(VEK >= 18) %>%
                                             mutate(cond = DATUKO < data_rheuma_and_psychiatric_dg$DATPRI[match(RODCIS2, data_rheuma_and_psychiatric_dg$RODCIS2)]) %>%
                                             filter(is.na(cond) | cond == TRUE) %>%
                                             select(-cond) 
                                           
                                           hospitalizations_unexp_no_mental_disorder_1999_2012 <- hospitalizations_unexp_1999_2012 %>%
                                             anti_join(hospitalizations_unexp_1999_2012 %>%
                                                         ...
                                                         inner_join(hospitalizations_1994_2017 %>%
                                                                      ...
                                                                      transmute(RODCIS2,
                                                                                ...
                                                                                historic_hospitalization_end = ymd(DATUKO)),
                                                                    by = c("RODCIS2" = "RODCIS2")) %>%
                                                         mutate(past_psychiatric_or_rheuma_hospitalizations = int_overlaps(interval(washout_start, washout_end),
                                                                                                                           interval(historic_hospitalization_start, historic_hospitalization_end))) %>%
                                                         group_by(RODCIS2, rowid) %>%
                                                         filter(any(past_psychiatric_or_rheuma_hospitalizations == TRUE)) %>%
                                                         ungroup(),
                                                       by = c("RODCIS2" = "RODCIS2", 
                                                              "rowid" = "rowid" ))
                                           
                                           matched_pairs <- data_rheuma_and_psychiatric_dg %>%
                                             transmute(RODCIS2_exposed = RODCIS2,
                                                       VEK,
                                                       POHL,
                                                       month_discharge = month(ymd(DATUKO)),
                                                       year_discharge = year(ymd(DATUKO))) %>%
                                             inner_join(hospitalizations_unexp_no_mental_disorder_1999_2012 %>%
...
map2(.x = map(first_hospitalizations_rheuma_no_mental_disorder_1999_2012,
              function(data_rheuma_dg) {
                
                map_int(data_rheuma_dg,
                        function(data_rheuma_and_psychiatric_dg) {
                          
                          nrow(data_rheuma_and_psychiatric_dg)
                        })
              }),
...
cohorts_baseline <- map2(first_hospitalizations_rheuma_no_mental_disorder_1999_2012,
                         all_matched_sampled_pairs,
                         function(data_rheuma_dg, data_rheuma_dg_matched_counterparts) {
                           
                           map2(data_rheuma_dg,
                                data_rheuma_dg_matched_counterparts,
                                function(data_rheuma_and_psychiatric_dg, data_rheuma_and_psychiatric_dg_matched_counterparts) {
                                  ...
                                  data_rheuma_and_psychiatric_dg %>%
                                    mutate(RODCIS2_exposed = RODCIS2,
                                           ...
                                           month_discharge = month(ymd(DATUKO)),
                                           year_discharge = year(ymd(DATUKO))) %>%
                                    bind_rows(data_rheuma_and_psychiatric_dg_matched_counterparts %>%
                                              ...
                                              data_rheuma_and_psychiatric_dg %>%
                                                ...
                                                inner_join(hospitalizations_1994_2017 %>%
                                                             transmute(RODCIS2,
                                                                       ...
                                                                       DDG5_follow_up = DDG5),
                                                           by = c("RODCIS2" = "RODCIS2")) %>%
                                                mutate(overlap = int_overlaps(interval(historic_hospitalization_start, historic_hospitalization_end),
...
data_washout <- map(cohorts_baseline_with_outcome,
                    function(data_rheuma_dg) {
...
save(data_washout, 
     file = "path/AZV_rheuma_risk_of_psych_hosp/Data/data_washout.RData")