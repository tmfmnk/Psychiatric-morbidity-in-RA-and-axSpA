...
imap_dfr(data_main_analysis,
  ...
  arrange(order) %>%
  select(-order) %>%
  write.csv(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/Abs_risk_main_analysis.csv",
            row.names = FALSE)
...
mfull %>%
  ...
  select(-order) %>%
  write.csv(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/HR_main_analysis.csv",
            row.names = FALSE)
...
p2_ra_and_as <- mfull %>%
  ...
  geom_hline(yintercept = 1, 
             linetype = 2) +
  scale_y_log10(breaks = c(0.25, 0.5, 0.7, 1, 1.5, 2, 3, 5, 7),
                limits = c(0.25, 7)) +
  coord_flip() +
  labs(x = NULL,
       y = "aHR (95% CI)") +
...
ggsave(file = "path/AZV_rheuma_risk_of_psych_hosp/Results/HR_main_analysis.eps",
       device = cairo_ps,
       width = 40,
       height = 55,
       units = "cm",
           ...
           imap_dfr(data_rheuma_dg,
                    function(data_rheuma_and_psychiatric_dg, outcome_names) {
                      ...
                      p <- ggsurvplot(surv_fit(Surv(years_until_outcome_or_censoring, outcome) ~ exposure, 
                                      ...
                                      title = paste0(outcome_names, " following ", tolower(rheuma_cohort_names)),
                      ...
                      ggsave(plot = plots,
                             filename = paste0("Cumulative_event_plot_", tolower(outcome_names), "_", tolower(rheuma_cohort_names), "_main_analysis", ".eps"),
